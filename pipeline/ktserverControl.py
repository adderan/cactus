...
###############################################################################
def runKtserver(dbElem, fileStore, killSwitchID, maxPortsToTry=100, readOnly = False,
                createTimeout=30, loadTimeout=10000, killTimeout=518400,
                killPingInterval=5):
    assert fileStore.globalFileExists(killSwitchID)
    if not fileStore.globalFileExists(killSwitchID):
        raise RuntimeError("Kill switch file not found, can't " +
                           "launch without it %s" % killSwitchID)
    dbPathExists = False
    ...
    try:        
        for port in xrange(basePort, basePort + maxPortsToTry):
            ...
            if os.path.exists(logPath):
                os.remove(logPath)
            if __isKtServerOnTakenPort(dbElem, fileStore, killSwitchID, pretest=True):
                continue
            ...
            procWaiter.start()
            assert fileStore.globalFileExists(killSwitchID)
            __writeStatusToSwitchFile(dbElem, process.pid, fileStore, killSwitchID)
            success = __validateKtserver(process, dbElem, fileStore, killSwitchID,
...
###############################################################################
def __validateKtserver(process, dbElem, fileStore, killSwitchID,
                       createTimeout, loadTimeout):
    success = False
    for i in xrange(createTimeout):
        ...
        if __isKtServerFailed(dbElem) or __isKtServerOnTakenPort(dbElem,
                                                                 fileStore, killSwitchID):
            success = False
            break
        if __isKtServerRunning(dbElem, fileStore, killSwitchID):
            success = True
            break
...
###############################################################################
def __writeStatusToSwitchFile(dbElem, serverPid, fileStore, killSwitchID):
    try:
        ...
        import traceback; traceback.print_exc(file=sys.stdout)
        raise RuntimeError("Unexpected error updating killswitch file %s" %
                           killSwitchID)
...
# able to be read. 
###############################################################################
def __readStatusFromSwitchFile(dbElem, serverPidAsList, fileStore, killSwitchID):
    try:
        ...
        port = None
        serverPid = None
        with fileStore.readGlobalFileStream(killSwitchID) as switchFile:
...
###############################################################################
def blockUntilKtserverIsRunnning(dbElem, fileStore, killSwitchID, timeout=518400,
                                 timeStep=10):
    logPath = __getLogPath(dbElem)
...
# information of the server
###############################################################################
def killKtServer(dbElem, fileStore, killSwitchID, killTimeout=10000):
    if not fileStore.globalFileExists(killSwitchID):
        raise RuntimeError("Can't kill server because file" +
                           " not found %s" % killSwitchID)
    logPath = __getLogPath(dbElem)
    isRunning =  __isKtServerRunning(dbElem, fileStore, killSwitchID)
    fileStore.deleteGlobalFile(killSwitchID)
    logPath = __getLogPath(dbElem)
    if not isRunning:
...
# information of the server
###############################################################################
def __isKtServerRunning(dbElem, fileStore, killSwitchID):
    logPath = __getLogPath(dbElem)
    success = False
    serverPidAsList = []
    ...
    if serverPidFromLog != serverPidAsList[0]:
        raise RuntimeError("Pid %s != %s (former from %s, lastter %s)" % (
            ...
            logPath, fileStore, killSwitchID))
...
###############################################################################
def __isKtServerOnTakenPort(dbElem, fileStore, killSwitchID, pretest = False):
