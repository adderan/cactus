...
def addKtserverDependentChild(rootJob, rootJobFileStore, newChild, maxMemory, maxCpu,
    ...
    killSwitchPath = getTempFile(rootDir=rootJobFileStore.getLocalTempDir())
    with open(killSwitchPath, 'w') as killSwitchFile:
        killSwitchFile.write("init")
    killSwitchID = rootJobFileStore.writeGlobalFile(killSwitchPath)
    ...
    rootJob.addChild(
        KtserverJobLauncher(dbElem, killSwitchID, maxMemory,
                               maxCpu, createTimeout,
                               loadTimeout, runTimeout, runTimestep))
    rootJob.addChild(
        KtserverJobBlocker(killSwitchID, newChild, isSecondary,
...
class KtserverJobLauncher(Job):
    ...
    def run(self, fileStore):
        fileStore.logToMaster("Launching ktserver %s with killSwitch file ID %s" % (
            ET.tostring(self.dbElem.getDbElem()), self.killSwitchID))
        runKtserver(self.dbElem, fileStore, self.killSwitchID,
...
class KtserverJobBlocker(Job):
    def __init__(self, killSwitchID, newChild, isSecondary,
                 blockTimeout, blockTimestep, killTimeout):
        Job.__init__(self)
        self.killSwitchID = killSwitchID
    ...
    def run(self, fileStore):
        ...
        fileStore.logToMaster("Blocking on ktserver %s with killPath %s" % (
            ET.tostring(dbElem.getDbElem()), self.killSwitchID))
            
        blockUntilKtserverIsRunnning(dbElem, fileStore, self.killSwitchID,
...
class KtserverJobKiller(Job):
    def __init__(self, dbElem, killSwitchID, killTimeout):
        Job.__init__(self)
        self.dbElem = dbElem
        self.killSwitchID = killSwitchID
        self.killTimeout = killTimeout
    ...
    def run(self, fileStore):
        ...
        killKtServer(self.dbElem, fileStore, self.killSwitchID,
