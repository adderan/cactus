...
class PreprocessChunk(Job):
    ...
    """
    def __init__(self, prepOptions, seqIDs, proportionSampled, inChunkID, outChunkID):
    ...
    def run(self, fileStore):
        ...
        outChunk = fileStore.readGlobalFile(self.outChunkID)
...
class MergeChunks(Job):
    ...
    """
    def __init__(self, prepOptions, chunkIDList, outSequenceID):
    ...
    def run(self, fileStore):
        ...
        outSequencePath = getTempFile(rootDir=fileStore.getLocalTempDir())
        popenPush("cactus_batch_mergeChunks > %s" % outSequencePath, " ".join(chunkList))
        fileStore.updateGlobalFile(self.outSequenceID, outSequencePath)
...
class BatchPreprocessor(Job):
    def __init__(self, prepXmlElems, inSequenceID, 
                 globalOutSequenceID, iteration = 0):
        Job.__init__(self) 
    ...
    def run(self, fileStore):
        ...
        if lastIteration == False:
            ...
            outSeqID = self.globalOutSequenceID
        
        if prepOptions.chunkSize <= 0: #In this first case we don't need to break up the sequence
            self.addChild(PreprocessChunk(prepOptions, [ self.inSequenceID ], 1.0, self.inSequenceID, outSeqID))
        else:
            self.addChild(PreprocessSequence(prepOptions, self.inSequenceID, outSeqID)) 
        
        if lastIteration == False:
            self.addFollowOn(BatchPreprocessor(self.prepXmlElems, outSeqID,
                                                     self.globalOutSequenceID, self.iteration + 1))
        else:
            self.addFollowOn(BatchPreprocessorEnd(self.globalOutSequenceID))
...
class CactusPreprocessor2(Job):
    ...
    def run(self, fileStore):
        ...
        if len(prepXmlElems) == 0: #Just cp the file to the output file
            ...
            #The input sequence file is now in the temporary folder for this job, so
            #it must be moved into the global fileStore before the next job.
